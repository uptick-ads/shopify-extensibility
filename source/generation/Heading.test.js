...
function mountWithRoot(extra = {}) {
  const generatedComponent = generateHeading(merge(base, extra));
  const root = mount((root) => {
    return createRoot(root).render(generatedComponent);
  });
  const rawComponent = root.find(Heading);
...
describe("Generates Heading Component", () => {
  test("creates with key and text", () => {
    ...
    expect(component.is(Heading)).toBeTruthy();
    expect(component.text()).toBe("Custom");
    // This isn't working since I had to hack it together
    // expect(component.tree.key).toBe("heading-test-1");
